...
states = {
    ...
    'stat_type_val': "Kills",
    'role_val': "Rifler" # NEW: Weapon/Role state
}
    ...
    if st.button("GET AI SLIDER ADVICE"):
        ...
        if not api_key:
            ...
            with st.spinner("AI analyzing deep context..."):
                # ADVANCED HS PROMPT: Weighing Role and Map Geometry
                prompt = f"""
                Act as an Esports Betting Syndicate Analyst.
                Match: {st.session_state.m_context_val} | Player: {st.session_state.p_tag_val}
                Prop Type: {st.session_state.stat_type_val} | Role: {st.session_state.role_val}
                
                ANALYTICAL CONTEXT:
                - Rank: {st.session_state.opp_rank_val} | Projected Maps: {st.session_state.expected_maps_val}
                - Map 1 Rate: {st.session_state.map1_rate} | Map 2 Rate: {st.session_state.map2_rate}
                
                CONSIDERATIONS: 
                If Prop is Headshots, weigh the 'Map Fit' slider heavily based on map geometry (long sightlines vs close quarters) and weapon choice ({st.session_state.role_val}).
...
with col_l:
    ...
    c1, c2 = st.columns(2)
    with c1:
        m_line, m_side = st.number_input("Line", value=35.5, step=0.5), st.selectbox("Side", ["Over", "Under"])
    with c2:
        m_odds, m_scope = st.number_input("Odds", value=-128), st.selectbox("Scope", ["Maps 1 & 2", "Map 1 Only", "Full Match"])
...
if st.button("RUN ELITE ANALYSIS"):
    try:
        ...
        proj = (base_rate * 21.5 * mapping.get(m_scope, 2.0)) * st.session_state.h2h_val * st.session_state.tier_val * st.session_state.map_val * st.session_state.int_val
        ...
        if st.checkbox("📸 Generate Social Share Card"):
            ...
            <div style="background: linear-gradient(145deg, #0e1117 0%, #1c2128 100%); border: 3px solid {grade_flat}; border-radius: 20px; padding: 30px; max-width: 480px; color: white; margin: 10px auto; text-align: center; box-shadow: 0 15px 35px rgba(0,0,0,0.6); font-family: sans-serif;">
                <div style="border-bottom: 2px solid rgba(255,255,255,0.1); padding-bottom: 15px; margin-bottom: 20px;">
                    <div style="font-size: 10px; color: #adbac7; text-transform: uppercase; letter-spacing: 3px;">{res.get('game', 'CS2')} {res.get('stat_label', 'Kills').upper()} ANALYSIS</div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; text-align: left;">
                    <div style="flex: 1;"><div style="font-size: 13px; color: #adbac7; font-weight: bold;">THE LINE</div><div style="font-size: 60px; font-weight: 900; line-height: 0.9; margin: 8px 0; letter-spacing: -2px;">{line_val}</div><div style="font-size: 30px; font-weight: 900; color: {arrow_hex};">{arrow_sym} {side_up.upper()}</div></div>
                    <div style="flex: 1; text-align: right;"><div style="font-size: 13px; color: #adbac7; font-weight: bold;">MODEL GRADE</div><div style="font-size: 115px; font-weight: 900; background: {grade_grad}; -webkit-background-clip: text; -webkit-text-fill-color: transparent; line-height: 0.8;">{res.get('grade', '?')}</div></div>
                </div>
                <div style="background: {grade_grad}; border-radius: 12px; padding: 18px; margin-bottom: 25px;"><div style="font-size: 36px; font-weight: 900; letter-spacing: -1px;">{res.get('units', 0)} UNIT PLAY</div></div>
                ...
                <div style="margin-top: 30px; font-size: 11px; color: #adbac7; text-transform: uppercase; letter-spacing: 4px; font-weight: 800; opacity: 0.7;">ANALYSIS BY <span style="color: white;">SLEEPER D. KID</span></div>
            </div>
            """, unsafe_allow_html=True)