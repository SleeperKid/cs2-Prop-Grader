...
<style>
    ...
    .hiro-grade { font-size: 100px; font-weight: 900; color: #FFD700; text-shadow: 0 0 20px rgba(255,215,0,0.7); }
...
# General State
keys = ['p_tag', 'l10', 'kpr', 'adr', 'acs', 'm_context', 'w_rank', 'ai_advice', 'tourney_type', 'proj_maps', 'results']
for key in keys:
...
with st.sidebar:
    ...
    if st.button("CONSULT AI ADVISOR"):
        ...
        if api_key:
            ...
            STRICT RULE: DO NOT hallucinate match history or H2H scores. Use Archetypes.
            CONTEXT: {st.session_state.m_context} | {st.session_state.tourney_type}.
            MAPS: {found_maps} | STRATEGY VAULT: {found_styles}
...
with col_l:
    ...
    c1, c2 = st.columns(2)
    with c1: 
        m_line = st.number_input("Prop Line", value=35.5, min_value=0.0, step=0.5)
        m_side = st.selectbox("Side", ["Over", "Under"])
    with c2: 
        # DEFAULT ODDS SET TO -128
        m_odds = st.number_input("Odds (PrizePicks Default)", value=-128)
...
if st.button("🚀 GENERATE ELITE GRADE"):
    ...
    # --- UPDATED MULTIPLIERS FOR PRIZEPICKS ---
    # MR12 = 24 rounds per map.
    game_mult = 26 if game_choice == "Valorant" else 24
    scope_map = {
        "Map 1 Only": 1.0,     # 1 Map (24-26 rounds)
        "Maps 1 & 2": 2.0,     # 2 Maps (48-52 rounds)
        "Full Match": 2.6      # ~2.5 Maps
    }
...
if st.session_state.results:
    ...
    with col_r:
        st.markdown(f"""<div class="map-logic-box"><b style="color:#58a6ff;">🗺️ PROJECTED MAPS LOGIC</b><br>Baseline (Scope Adj): <b>{res['base']:.1f}</b> | Weighted: <b>{res['proj']:.1f} Kills</b></div>""", unsafe_allow_html=True)
