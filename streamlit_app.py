...
# ==========================================
# ⚙️ SIDEBAR: THE AI ADVISOR (CONSISTENT)
# ==========================================
with st.sidebar:
    ...
    if st.button("GET AI SLIDER ADVICE"):
        ...
        if not api_key:
            ...
            with st.spinner("Analyzing Matchup Context..."):
                prompt = f"""
                Act as a Professional Esports Betting Model. Provide reasoning and weights.
                
                CONTEXT:
                - PLAYER: {st.session_state.p_tag_val} | TEAM: {p_team}
                - OPPONENT: {o_team} | OPP_RANK: {st.session_state.opp_rank_val}
                
                MATCH DATA:
                - PROP: {st.session_state.stat_type_val} | ROLE: {st.session_state.role_val} ({role_info})
                - MAP: {map_name} ({map_data})
                - DEEP STATS: Map1 Rate: {st.session_state.map1_rate} | Map2 Rate: {st.session_state.map2_rate} | Opening Duels: {st.session_state.opening_val}

                TASK:
                1. Assign 4 Weights (0.85 to 1.15) for our model. 
                2. Be precise: If Opponent Rank is stronger, increase 'Tier'. If map fit is high, increase 'Map'.
                3. FORMAT: H2H: [X] | Tier: [X] | Map: [X] | Int: [X]
                
                Provide a sharp, 2-sentence reasoning followed by the weights.
                """
                
                # Temperature 0.2: Sharp reasoning without slider drift
    ...
    if st.session_state.analysis_results:
        ...
        st.markdown(f"""
            <div class="grade-card" style="background: {grade_grad}; color: white;">
                ...
                <div style="font-size: 24px; margin-top: 10px; color: {arrow_hex}; font-weight: 900;">{arrow_sym} {side_up.upper()} {line_val} {res.get('stat_label', 'Kills').upper()}</div>
        ...
        if st.checkbox("📸 Generate Social Share Card"):
            # Rendered directly without intermediate variables to bypass "Magic" leaks
            st.markdown(f"""
