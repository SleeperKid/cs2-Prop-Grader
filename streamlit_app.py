...
# --- 🧠 AI & STATE ENGINE ---
def run_ai_advisor():
    intel = load_intel_vault().get(st.session_state.game_choice, {})
    ...
    if rank > 0 and rank <= 10:
        analysis["w_tier"] = 0.92
        analysis["notes"].append(f"Rank {rank}: Top tier defense detected.")
    
    for team, style in intel.get("team_styles", {}).items():
        if team.lower() in context:
            analysis["notes"].append(f"Intel: {style}")
...
defaults = {
    ...
    'w_tier': 1.0, 'w_map': 1.0, 'w_int': 1.0, 'results': None
}
...
with col_l:
    ...
    st.number_input("Opponent World Rank", key="opp_rank_input", min_value=1, step=1)
    ...
    if st.button("🚀 EXECUTE GRADING ENGINE", use_container_width=True):
        try:
            ...
            # 🟢 UPDATED MATH: RANK ADJUSTMENT
            rank_mod = 1.0
            if st.session_state.opp_rank_input <= 5: rank_mod = 0.93 # Tier 1 Wall
            elif st.session_state.opp_rank_input > 30: rank_mod = 1.07 # Farming Rank
            
            weights = st.session_state.w_h2h * st.session_state.w_tier * st.session_state.w_map * st.session_state.w_int * rank_mod
            ...
            # 🟢 S-GRADE TIGHTENING: Require 22% edge AND 70% Hit Rate
            grade = "B"
...
with col_r:
    if st.session_state.results:
        ...
        # 🟢 THE VISUAL GRADE PLATE
        st.markdown(f"""
        <div style="background:#1a1c23; border: 1px solid #333; border-radius: 15px; padding: 20px; text-align:center;">
            <div style="color:#888; font-size:12px; font-weight:bold;">INTERNAL MODEL DECISION</div>
            <div style="font-size: 100px; font-weight: 900; color: #FFD700; text-shadow: 0 0 20px rgba(255,215,0,0.4); line-height:1;">{res['grade']}</div>
        ...
        if st.checkbox("💎 Generate Social Media Card"):
            st.markdown(f"""
            <div style="background-color:#121212; border:2px solid #FFD700; border-radius:20px; padding:35px; width:450px; margin:auto; color:white; text-align:center;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <div style="text-align:left;">
                        ...
                        <span style="color:{'#00FF00' if res['side'] == 'Over' else '#FF0000'}; border:1px solid {'#00FF00' if res['side'] == 'Over' else '#FF0000'}; padding:4px 10px; border-radius:8px; font-weight:900;">{res['side'].upper()}</span>
                    </div>
                    <div style="text-align:center;">
                        <div style="color:#888; font-size:11px;">GRADE</div>
