...
with st.sidebar:
    ...
    if st.button("CONSULT AI ADVISOR"):
        ...
        if api_key:
            ...
            # --- RIGID DETERMINISTIC PROMPT ---
            prompt = f"""
            SYSTEM ROLE: Rigid Pro Betting Analyst.
            TEMPERATURE SETTING: 0.01 (High Precision).
            CONTEXT: {st.session_state.m_context} | {st.session_state.tourney_type}.
            PROP TYPE: {st.session_state.get('active_stat_type', 'KILLS')}.
            MAPS: {found_maps} | VAULT DATA: {found_styles}.
            
            TASK: Suggest 4 weights (0.85-1.15) for H2H, Tier, Map, Int based on Archetypes.
            GUIDELINES: 
            1. Be extremely concise. Max 3-4 sentences per decision.
            2. For HEADSHOTS, adjust volatility/map weights accordingly.
            3. You MUST include weights in brackets: [1.05].
            4. Remain logically identical even if asked multiple times.
            """
...
with col_l:
    ...
    c1, c2 = st.columns(2)
    with c1: 
        # --- FIXED TYPEERROR WITH KEYWORD ARGS ---
        m_line = st.number_input("Prop Line", value=float(35.5), min_value=0.0, step=0.5) #
        m_side = st.selectbox("Side", ["Over", "Under"])
    with c2: 
        m_odds, m_scope = st.number_input("Odds", value=int(-128)), st.selectbox("Scope", ["Maps 1 & 2", "Map 1 Only", "Full Match"])
