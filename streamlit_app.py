...
<style>
    ...
    .context-box { background: #1c2128; padding: 20px; border-radius: 15px; border: 1px solid #30363d; margin-top: 20px; }
</style>
""", unsafe_allow_html=True)

# ==========================================
# 🧠 STATE MANAGEMENT (FIXES THE CHECKBOX BUG)
# ==========================================
df = load_vault()
INTEL = load_intel()

keys = ['p_tag', 'l10', 'kpr', 'm_context', 'w_rank', 'results', 'ai_advice']
for key in keys:
...
with st.sidebar:
    ...
    if st.button("CONSULT AI ADVISOR"):
        ...
        if api_key:
            ...
            p_notes = INTEL.get(st.session_state.p_tag, {}).get('notes', "No notes.")
            prompt = f"""Analyze {st.session_state.m_context} (Opponent Rank: {st.session_state.w_rank}).
            Player Intel: {p_notes}. Suggest 4 weights (0.85-1.15) for: H2H, Tier, Map, Intensity.
            EXPLAIN EACH WEIGHT IN EXACTLY 4 SENTENCES. 
            Format: Weight: [X] | Explanation: [4 Sentences]"""
    ...
    # 📖 RESTORED SIDEBAR GUIDE
    with st.expander("📖 SLIDER STRATEGY GUIDE"):
        st.write("**H2H:** Boost if player dominates this specific opponent style.")
        st.write("**Tier:** Reduce vs Top 5 teams; Boost vs Tier 2.")
        st.write("**Map:** Adjust based on recent map-specific KPR.")
        st.write("**Intensity:** Boost for LAN/Elimination matches.")
...
with col_l:
    # 🧬 DEEP PROFILE SECTION (RESTORED)
    if st.session_state.p_tag in INTEL:
        st.markdown(f"""<div class="intel-box"><span style="color:#58a6ff;font-weight:bold;">🔍 DEEP SCOUT: {st.session_state.p_tag.upper()}</span><br><p style="color:#adbac7;margin-top:5px;">{INTEL[st.session_state.p_tag].get('notes', '')}</p></div>""", unsafe_allow_html=True)
    ...
    st.session_state.w_rank = st.text_input("Opponent World Rank", value=st.session_state.w_rank, placeholder="e.g. #3 Global")
...
if st.button("🚀 RUN ELITE ANALYSIS"):
    ...
    grade_char = "S" if edge >= 12 else "A+" if edge >= 8 else "A" if edge >= 3 else "B"
    units = 2.5 if edge >= 12 else 2.0 if edge >= 8 else 1.0 if edge >= 3 else 0.5

    st.session_state.results = {"grad": grad, "grade": grade_char, "units": units, "proj": final_proj, "base": base_proj, "edge": edge, "prob": prob, "vals": vals, "m_line": m_line, "m_side": m_side}

# --- RESULTS DISPLAY (OUTSIDE BUTTON TO FIX CHECKBOX BUG) ---
if st.session_state.results:
    res = st.session_state.results
    with col_r:
        st.markdown(f"""<div class="grade-card" style="background: {res['grad']};">
            <div style="font-size:20px; font-weight:bold; color:white;">ELITE ANALYSIS</div>
        ...
        # 🗺️ PROJECTED MAPS BOX (RESTORED)
        st.markdown(f"""<div class="context-box"><b style="color:#58a6ff;">🗺️ PROJECTION BREAKDOWN</b><br>
            Baseline Proj: {res['base']:.1f} | Weighted Final: <b>{res['proj']:.1f}</b><br>
            Model Edge: <b>{res['edge']:.1f}%</b> | Win Probability: <b>{res['prob']:.1f}%</b></div>""", unsafe_allow_html=True)
