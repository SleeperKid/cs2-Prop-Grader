...
<style>
    ...
    .share-container {
        ...
        font-family: 'Arial', sans-serif; box-shadow: 0 0 20px rgba(255, 215, 0, 0.2);
    }
    .share-header { font-size: 14px; letter-spacing: 3px; opacity: 0.7; text-transform: uppercase; }
    .share-player { font-size: 50px; font-weight: 900; margin: 10px 0; letter-spacing: 2px; }
    .share-match { color: #58a6ff; font-weight: bold; font-size: 18px; margin-bottom: 15px; }
    .share-rule { border-top: 1px solid #30363d; margin: 15px 0; }
    ...
    .grade-box { font-size: 100px; font-weight: 900; color: #FFD700; text-shadow: 0 0 15px rgba(255,215,0,0.5); line-height: 1; }
    ...
    .metric-item { font-size: 10px; opacity: 0.7; }
...
with st.sidebar:
    ...
    if st.button("CONSULT AI ADVISOR"):
        ...
        if api_key:
            ...
            prompt = f"""Expert Mode. Context: {st.session_state.m_context} | {st.session_state.tourney_type} | Player: {st.session_state.p_tag}. 
            Stats: {stat_context}. Intel: {p_intel}.
            Task: Weights for H2H, Tier, Map, Int. Max 4 sentences each.
            Format numeric weights as [1.05]."""
    ...
    # 📖 RESTORED SLIDER GUIDE
    with st.expander("📖 SLIDER STRATEGY GUIDE"):
        st.write("**H2H:** Boost for favorable player matchups.")
        st.write("**Tier:** Lower vs Elite Top 5 teams.")
        st.write("**Map:** Use Intel map explanations to adjust.")
        st.write("**Intensity:** Boost for Playoffs/Grand Finals.")
...
if st.button("🚀 RUN ELITE ANALYSIS"):
    ...
    st.session_state.results = {"grade": "S" if edge >= 12 else "A+" if edge >= 8 else "A" if edge >= 3 else "B", "units": 2.5 if edge >= 12 else 2.0 if edge >= 8 else 1.0 if edge >= 3 else 0.5, "proj": final_proj, "base": base_proj, "edge": edge, "prob": prob, "m_line": m_line, "m_side": m_side, "scope": m_scope, "hit": hit_rate}

if st.session_state.results:
    res = st.session_state.results
    with col_r:
        # 🗺️ RESTORED MAP LOGIC BOX
        ...
        if st.checkbox("Show Social Share Card"):
            ...
            <div class="share-container">
                <div class="share-header">{game_choice.upper()} PROP ANALYSIS</div>
                <div class="share-player">{st.session_state.p_tag.upper()}</div>
                <div class="share-match">{st.session_state.m_context}</div>
                <div class="share-rule"></div>
                <div style="display: flex; justify-content: space-around; align-items: center;">
                    <div style="text-align: left;">
                        <small>THE PROP LINE</small><br>
                        <b style="font-size: 40px;">{res['m_line']}</b><br>
                        <small>KILLS</small>
                    </div>
                    <div>
                        <small>MODEL GRADE</small><br>
                ...
                <div class="metric-grid">
                    <div class="metric-item">PROJ<br><b class="metric-val">{res['proj']:.1f}</b></div>
                    <div class="metric-item">EDGE<br><b class="metric-val">+{res['edge']:.1f}%</b></div>
                    <div class="metric-item">CONF<br><b class="metric-val">{res['prob']:.0f}%</b></div>
                    <div class="metric-item">L10 HIT<br><b class="metric-val">{res['hit']:.0f}%</b></div>
                </div>
